document.addEventListener("DOMContentLoaded", async () => {
  ...
  function filteredVideos() {
    ...
    return state.videos.filter((video) => {
      ...
      return (video.original_filename || "").toLowerCase().includes(searchQuery);
    });
  }
  ...
  function renderVideoPickerList() {
    ...
    videoPickerBody.innerHTML = videos.map((video) => {
      ...
      return `
        <div class="cl-picker-item">
          ...
          <div class="cl-picker-info">
            <div class="cl-picker-name">${app.escapeHtml(video.original_filename || "-")}</div>
          ...
          >
            ${isSelected
              ? `<i class="ti ti-check me-1" aria-hidden="true"></i>Selected`
              : "Select"}
          </button>
        </div>
      `;
  ...
  const thumbShell = document.getElementById("clSourceThumbShell");
  const thumbImg = document.getElementById("clSourceThumbImg");
  ...
  function renderSelectedVideoSummary() {
    ...
    if (!video) {
      ...
      openVideoPickerButton.innerHTML = `<i class="ti ti-search me-1" aria-hidden="true"></i> Select Video`;
      goToAnalysisButton.href = "/analysis";
      if (thumbShell) thumbShell.classList.add("is-fallback");
      return;
    }
    ...
    `;
    openVideoPickerButton.innerHTML = `<i class="ti ti-switch me-1" aria-hidden="true"></i> Change Video`;
    goToAnalysisButton.href = `/analysis?video_id=${video.id}`;
    if (thumbImg) {
      thumbImg.src = thumbnailUrl(video);
      thumbImg.alt = app.escapeHtml(video.original_filename);
      if (thumbShell) {
        thumbShell.classList.remove("is-fallback");
        thumbImg.onerror = () => thumbShell.classList.add("is-fallback");
      }
    }
  }
  ...
  function setActiveLineOrder(lineOrder) {
    ...
    lineOrderOneButton.className = `cl-line-card cl-tab--line1${lineOrder === 1 ? " cl-line-card--active" : ""}`;
    lineOrderTwoButton.className = `cl-line-card cl-tab--line2${lineOrder === 2 ? " cl-line-card--active" : ""}`;
    const label = lineOrder === 1 ? "Line 1" : "Line 2";
    countLinesModeText.textContent = `Active: ${label} — click two points on the video.`;
    renderCanvas();
  }
  ...
  const playPauseBtn   = document.getElementById("clPlayPauseBtn");
  const seekBar        = document.getElementById("clSeekBar");
  const currentTimeEl  = document.getElementById("clCurrentTime");
  const durationEl     = document.getElementById("clDuration");
  const fullscreenBtn  = document.getElementById("clFullscreenBtn");

  function ctrlFormatTime(s) {
  ...
  function syncPlayPauseIcon() {
    ...
    if (icon) icon.className = videoPlayer.paused
  ...
  videoPlayer.addEventListener("loadedmetadata", () => {
    ...
    if (seekBar)    { seekBar.max = dur; seekBar.value = 0; }
    if (durationEl) durationEl.textContent = ctrlFormatTime(dur);
  });

  videoPlayer.addEventListener("timeupdate", () => {
    if (seekBar && !seekBar._seeking) seekBar.value = videoPlayer.currentTime;
    if (currentTimeEl) currentTimeEl.textContent = ctrlFormatTime(videoPlayer.currentTime);
  });
  ...
  if (seekBar) {
    ...
    seekBar.addEventListener("input", () => {
      if (currentTimeEl) currentTimeEl.textContent = ctrlFormatTime(Number(seekBar.value));
    });
